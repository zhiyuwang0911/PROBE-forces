"""
Extended XYZ I/O for PROBE datasets with precomputed MACE predictions.

Expected frame format (per structure):
    Properties=species:S:1:pos:R:3:forces:R:3:MACE_forces:R:3 ...
    <info line with energy=... MACE_energy=...>

Atom line columns: species  x  y  z  fx  fy  fz  mace_fx  mace_fy  mace_fz
"""
...
def read_probe_extxyz_frame(lines: list[str], start: int = 0) -> tuple[dict, int]:
    ...
    frame = {
        ...
        'mace_forces': np.asarray(mace_forces, dtype=np.float64),
        'true_energy': _parse_float(info.get('energy')),
        'mace_energy': _parse_float(info.get('MACE_energy')),
...
def iter_probe_extxyz(path: str | Path,
    ...
    """Yield frame dicts from a PROBE-style extended XYZ file."""
    path = Path(path)
...
def load_probe_extxyz(path: str | Path,
    ...
    """Load all frames from a PROBE-style extended XYZ file."""
